    ...
    private static int dijkstra(int start,int end) {
        ...
        PriorityQueue<node> pq = new PriorityQueue((o1,o2)->{
            if(((node)o1).sum == ((node)o2).sum){
                return ((node)o1).now - ((node)o2).now;
            }
        ...
        while(!pq.isEmpty()){
            ...
            for(int[] next : map[now.now]){
                if(hm.containsKey(next[0])) continue;
                if(dist[next[0]] >= now.sum+next[1]) {
                    dist[next[0]] = now.sum+next[1];
                    ArrayList<Integer> temp = (ArrayList<Integer>) now.hist.clone();
                    pq.add(new node(next[0], dist[next[0]] , temp));
                }
            }
        }
